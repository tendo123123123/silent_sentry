...
class OdomGroundTruthComparator(Node):
    def __init__(self):
        ...
        self.loc_initial_x = None
        self.loc_initial_y = None
        self.loc_initial_yaw = None
        ...
        self.raw_initial_x = None
        self.raw_initial_y = None
        self.raw_initial_yaw = None
    ...
    def gt_callback(self, msg: Pose):
        ...
        self.gt_yaw = quaternion_to_yaw(msg.orientation)
        ...
        if self.gt_initial_x is None:
            ...
            self.reference_yaw = self.gt_yaw
        ...
        if not self.gt_received:
            self.get_logger().info(
                f'GT data flowing: x={self.gt_x:.2f} y={self.gt_y:.2f} yaw={math.degrees(self.gt_yaw):.1f}°'
            )
    ...
    def raw_callback(self, msg: Odometry):
        ...
        self.raw_yaw = quaternion_to_yaw(msg.pose.pose.orientation)
        if self.raw_initial_x is None:
            self.raw_initial_x = self.raw_x
            self.raw_initial_y = self.raw_y
            self.raw_initial_yaw = self.raw_yaw
        self.raw_received = True

    def _get_localized_pose(self):
        return lookup_localized_pose(
            self.tf_buffer,
            self.ekf_x,
            self.ekf_y,
            self.ekf_yaw,
    ...
    def _compute_and_publish_errors(self):
        ...
        gt_rel_x, gt_rel_y, gt_rel_yaw = align_pose_to_reference(
            self.gt_x,
            self.gt_y,
            self.gt_yaw,
            self.gt_initial_x,
        ...
        # ---- Localized (map-frame) errors ----
        localized_pose = self._get_localized_pose()
        if localized_pose is None:
            return
        if self.loc_initial_x is None:
            self.loc_initial_x = localized_pose.x
            self.loc_initial_y = localized_pose.y
            self.loc_initial_yaw = localized_pose.yaw

        loc_x, loc_y, loc_yaw = align_pose_to_reference(
            localized_pose.x,
            localized_pose.y,
            localized_pose.yaw,
            self.loc_initial_x,
        ...
        ekf_head_err = abs(math.degrees(
            self._normalize_angle(loc_yaw - gt_rel_yaw)
        ))
        ...
        if self.raw_received:
            raw_rel_x, raw_rel_y, raw_rel_yaw = align_pose_to_reference(
                self.raw_x,
                self.raw_y,
                self.raw_yaw,
                self.raw_initial_x,
            ...
            raw_pos_err = math.sqrt(
                (raw_rel_x - gt_rel_x) ** 2 +
                (raw_rel_y - gt_rel_y) ** 2
            )
            raw_head_err = abs(math.degrees(
                self._normalize_angle(raw_rel_yaw - gt_rel_yaw)
            ))
    ...
    def publish_summary(self):
        ...
        raw_pos_err = float('nan')
        if self.raw_received:
            raw_rel_x, raw_rel_y, _ = align_pose_to_reference(
                self.raw_x,
                self.raw_y,
                self.raw_yaw,
                self.raw_initial_x,
